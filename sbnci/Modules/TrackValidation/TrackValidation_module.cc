...
class sbnd::TrackValidation : public art::EDAnalyzer {
public:
...
sbnd::TrackValidation::TrackValidation(fhicl::ParameterSet const& p)
  : EDAnalyzer{p},
...
void sbnd::TrackValidation::analyze(art::Event const& evt)
{
  ...
  // Associations
  art::FindManyP<recob::SpacePoint> fm_spoint(pfps,  evt, fSpacePointLabel);
  art::FindManyP<recob::Cluster>    fm_cluster(pfps, evt, fPFParticleLabel);
  art::FindManyP<recob::Shower>     fm_shower(pfps,  evt, fShowerLabel);
  art::FindManyP<recob::Track>      fm_track(pfps,   evt, fTrackLabel);
  art::FindManyP<recob::Hit>        fm_clusterhit(clusters, evt, fHitLabel);
  art::FindManyP<recob::Hit>        fm_showerhit(showers,   evt, fShowerLabel);
  art::FindManyP<recob::Hit>        fm_trackhit(tracks,     evt, fTrackLabel);
  art::FindManyP<anab::Calorimetry> fm_cal(tracks, evt, fCalLabel);
  art::FindManyP<anab::ParticleID>  fm_pid(tracks, evt, fPIDLabel);
  ...
  for(const art::Ptr<recob::PFParticle> &pfp : pfps){
    const std::vector<art::Ptr<recob::Shower>> pfpshowers = fm_shower.at(pfp.key());
    const std::vector<art::Ptr<recob::Track>> pfptracks = fm_track.at(pfp.key());
    if(!pfptracks.empty()){
      assert(pfptracks.size()==1);
      const std::vector<art::Ptr<recob::Hit>> temphits = fm_trackhit.at(pfptracks[0].key());
      pfphits.insert(pfphits.end(), temphits.begin(), temphits.end());
    }
    if(!pfpshowers.empty()){
      assert(pfpshowers.size()==1);
      const std::vector<art::Ptr<recob::Hit>> tempthits = fm_showerhit.at(pfpshowers[0].key());
  ...
  for(const art::Ptr<recob::PFParticle> &pfp : pfps){
    ...
    const std::vector<art::Ptr<recob::Track>> tracks = fm_track.at(pfp.key());
    if(tracks.empty()){
    ...
    const std::vector<art::Ptr<recob::SpacePoint>> spoints = fm_spoint.at(pfp.key());
    if(spoints.empty()){
    ...
    const std::vector< art::Ptr< recob::Cluster> > thisclusters = fm_cluster.at(pfp.key());
    ...
    for (const auto& cluster: thisclusters){
      ...
      // Get hits in cluster
      cluster_hits = fm_clusterhit.at(pfp.key());
    ...
    const std::vector<art::Ptr<recob::Hit>> trackhits = fm_trackhit.at(tracks[0].key());
    ...
    std::vector< art::Ptr<anab::Calorimetry> > trackcals = fm_cal.at(tracks[0].key());
    ...
    std::vector< art::Ptr<anab::ParticleID> > trackpids = fm_pid.at(tracks[0].key());
...
void sbnd::TrackValidation::beginJob()
{
...
int sbnd::TrackValidation::NumberOfSharedHits(int trackID, detinfo::DetectorClocksData const& clock_data, const std::vector<art::Ptr<recob::Hit>> &hits){
...
int sbnd::TrackValidation::NumberOfTrueHits(detinfo::DetectorClocksData const& clockData,
                                                 const std::vector<art::Ptr<recob::Hit>>& pHits,
...
void sbnd::TrackValidation::endJob()
{
  // Implementation of optional member function here.
}

DEFINE_ART_MODULE(sbnd::TrackValidation)
