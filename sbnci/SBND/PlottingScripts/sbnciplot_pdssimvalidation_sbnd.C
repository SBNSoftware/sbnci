...
void PlotTimeHist(TFile* inFile, TFile* outFile,
...
void sbnciplot_pdssimvalidation_sbnd(TString inputFile)
{
  ...
  PlotTimeHist(inFile, outFile, treeName, "NTimePhotonsMap",  "", "T_{SimPhoton} - T_{0} [ns]", histYaxis, 40, 0, 6000, "NTimePhotonsHist", "", dirName);  
                  ...
                  TString metricToPlot, TString condition, TString dirName)
{
  ...
  // Photon times are stored as integers in ns.
  // Their propagation time has two scales: ~10 ns, and ~1 us.

  // This hardcoded binning is not great, but is OK. The first 8 bins
  // handle the small scale, where the integer times are more
  // relevant; after that it's a regularised logarithmic binning.
  ...
  for (int iEntry = 0; metricTree->LoadTree(iEntry) >= 0; ++iEntry) {
    ...
    for(const auto& ntp : *mapp) {
      ...
      // this slow filling method is necessary to have the right number of entries
      if(ntp.first <= xMinFixed) for(int i=0;i<ntp.second;i++)hist->Fill(xMin);
      else if(ntp.first > xMax) for(int i=0;i<ntp.second;i++)hist->Fill(xMax);
      else for(int i=0;i<ntp.second;i++)hist->Fill(ntp.first);
      // This other filling method yields 0 entries =/
  ...
  TObjString* s1 = new TObjString("SetLogx");
  TList* histLists = hist->GetListOfFunctions();
  histLists->AddLast(s1);
